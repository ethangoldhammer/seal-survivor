...
const DOCS = {
  ...
  'bossPerks.csv': {
    ...
    damagePerDifficulty: 'Linear growth on `damage` per difficulty point, resolved ONCE when the perk is attached — the same shape enemies.csv\'s hpPerDifficulty has, and for the same reason. Blank means a flat perk. Flat is not neutral: everything else the boss does rides the spawn ramp, so a perk whose damage never moves is one that quietly stops mattering.',
