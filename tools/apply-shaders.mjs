...
// ignored.
export const MARK_START = '// >>> shader lab presets — generated by tools/apply-shaders.mjs, do not hand-edit';
...
export function braceBlock(masked, key, from = 0, to = masked.length) {
...
// there yet.
export function spliceFields(text, masked, block, fields, label, changes, stale) {
  const edits = [];
  const spans = fieldSpans(masked, block);
  const additions = [];
  for (const [key, value] of Object.entries(fields)) {
...
export function directChild(masked, key, [open, close]) {
  const re = new RegExp(`(^|[\\s,{])(${key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}|"${key}")\\s*:\\s*\\{`, 'gm');
  re.lastIndex = open + 1;
  let m;
  while ((m = re.exec(masked)) && m.index < close) {
...
export async function devServerBlocking(what, notes) {
  let running = [];
  try {
